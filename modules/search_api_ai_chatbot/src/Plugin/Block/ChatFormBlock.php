...
namespace Drupal\search_api_ai_chatbot\Plugin\Block;
...
use Drupal\search_api_ai_chatbot\Form\ChatForm;
 ...
 *
 * @Block(
 *   id = "search_api_ai_chat_foundation_form",
...
EOF,
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['chat']['first_message'] = [
      '#type' => 'textarea',
      '#title' => $this->t('First message'),
      '#description' => $this->t('The first message to show in the chat window, from the bot.'),
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
    ...
    $this->configuration['first_message'] = $form_state->getValue(['chat', 'first_message']);
  ...
  public function build() {
    ...
    $message = [
      '#theme' => 'search_api_ai_chatbot_message',
      '#username' => $this->configuration['bot_name'],
      '#timestamp' => date('H:i:s'),
      '#message' => $this->configuration['first_message'],
    ];

    $block['#theme'] = 'search_api_ai_chatbot';
    $block['#attached']['library'][] = 'search_api_ai_chatbot/chat';
    $block['#header'] = $this->configuration['label'];
    $block['#rendered_form'] = $form;
    $block['#messages'] = [$message];

    $block['#attached']['drupalSettings']['search_api_ai_chatbot']['bot_name'] = $this->configuration['bot_name'];
    $block['#attached']['drupalSettings']['search_api_ai_chatbot']['bot_image'] = $this->configuration['bot_image'];
    $block['#attached']['drupalSettings']['search_api_ai_chatbot']['default_username'] = $this->configuration['default_username'];
    $block['#attached']['drupalSettings']['search_api_ai_chatbot']['default_avatar'] = $this->configuration['default_avatar'];
    ...
    if ($user->isAuthenticated() && $this->configuration['use_username']) {
      $block['#attached']['drupalSettings']['search_api_ai_chatbot']['default_username'] = $user->getDisplayName();
    }
    ...
    if ($user->isAuthenticated() && $this->configuration['use_avatar']) {
      ...
      if (!empty($userEntity->user_picture->entity)) {
        $block['#attached']['drupalSettings']['search_api_ai_chatbot']['default_avatar'] = $userEntity->user_picture->entity->getFileUri();
      }
    }
