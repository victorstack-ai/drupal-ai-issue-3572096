 ...
 */
class AudioToAudioGenerationForm extends FormBase {
  ...
  public function getFormId() {
    return 'ai_api_explorer_audio_to_audio_prompt';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (!$this->providerManager->hasProvidersForOperationType('audio_to_audio')) {
      $form['markup'] = [
        '#markup' => '<div class="ai-error">' . $this->t('No AI providers are installed for Audio to Audio calls. Choose a provider from the <a href="@ai">AI module homepage</a>, add it to your project, then %install and %configure it first.', [
    ...
    $this->aiProviderHelper->generateAiProvidersForm($form, $form_state, 'audio_to_audio', 'ata', AiProviderFormHelper::FORM_CONFIGURATION_FULL);
  ...
  public function normalizeCodeExample(AiProviderInterface|ProviderProxy $provider, FormStateInterface $form_state, string $filename): string {
    ...
    $code .= "\$response = \$ai_provider->audioToAudio(\$input, '" . $form_state->getValue('ata_ai_model') . '\', ["your_module_name"]);<br><br>';
