...
class AiContentSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['analyse_policies_enabled'] = array(
      '#type' => 'select',
      ...
      '#description' => t('<em>AI can analyze content and tell you what content policies it may violate. This is beneficial if your audience are certain demographics and sensitive to certain categories. Note that this is only a useful guide.'),
      '#title' => t('Choose Content analysis sidebar widget model.'),
    );
    $form['tone_adjust_enabled'] = array(
      '#type' => 'select',
      '#options' => $chat_models,
      '#disabled' => count($chat_models) == 0,
      '#description' => t('Have AI check your content and adjust the tone of it for different reader audiences for you.'),
      '#default_value' => $config->get('tone_adjust_enabled') ?? $default_chat_model,
      '#title' => t('Choose Adjust content tone widget model.'),
    );
    $form['summarise_enabled'] = array(
      '#type' => 'select',
      '#options' => $chat_models,
      '#disabled' => count($chat_models) == 0,
      '#description' => t('This will use the selected field and OpenAI will attempt to summarize it for you. You can use the result to help generate a summary/teaser, social media share text, or similar.'),
      '#default_value' => $config->get('summarise_enabled') ?? $default_chat_model,
      '#title' => t('Choose Summarisation widget model.'),
    );
    $form['suggest_title_enabled'] = array(
      '#type' => 'select',
      '#options' => $chat_models,
      '#disabled' => count($chat_models) == 0,
      '#description' => t('AI can try to suggest an SEO friendly title from the selected field.'),
      '#default_value' => $config->get('suggest_title_enabled') ?? $default_chat_model,
      '#title' => t('Choose Title suggestion widget model.'),
    );
    $form['suggest_tax_enabled'] = array(
      '#type' => 'select',
      '#options' => $chat_models,
      '#disabled' => count($chat_models) == 0,
      '#description' => t('AI can attempt to suggest possible classification terms to use as taxonomy.'),
      '#default_value' => $config->get('suggest_tax_enabled') ?? $default_chat_model,
      '#title' => t('Choose Taxonomy term suggestion widget model.'),
    );
