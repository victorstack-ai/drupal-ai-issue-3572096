...
class AiOtelSpansEventSubscriber implements EventSubscriberInterface {
   ...
   *   The event to export.
   */
  public function handlePostGenerateResponseEvent(AiProviderResponseBaseEvent $event): void {
    $config = $this->configFactory->get(SettingsForm::CONFIG_NAME);
    if (
      !$config->get(SettingsForm::CONFIG_KEY_OTEL_ENABLED)
      || !$config->get(SettingsForm::CONFIG_KEY_OTEL_SPANS)
   ...
   */
  protected function fillRequestSpanAttributes(SpanInterface $span, AiProviderRequestBaseEvent $event): void {
   ...
   *   The event to get data from.
   */
  protected function fillResponseSpanAttributes(SpanInterface $span, AiProviderResponseBaseEvent $event): void {
