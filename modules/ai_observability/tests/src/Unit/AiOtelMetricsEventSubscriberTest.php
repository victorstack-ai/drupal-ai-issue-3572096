...
class AiOtelMetricsEventSubscriberTest extends UnitTestCase {
   ...
   */
  public function testWithOtelMetricsEnabled(): void {
    /** @var \Drupal\test_helpers\Stub\ConfigFactoryStub $configFactory */
    $configFactory = TestHelpers::service('config.factory');
    $configFactory->stubSetConfig(SettingsForm::CONFIG_NAME, [
   ...
   */
  private function initAiOtelMetricsEventSubscriberService(OpentelemetryMetrics $otelMetrics): AiOtelMetricsEventSubscriber {
    /** @var \Drupal\test_helpers\Stub\ConfigFactoryStub $configFactory */
    $configFactory = TestHelpers::service('config.factory');
    /** @var \Drupal\Core\Session\AccountProxyInterface|object $currentUser */
    $currentUser = TestHelpers::service('current_user');
    return new AiOtelMetricsEventSubscriber(
      $configFactory,
      $otelMetrics,
      // @phpstan-ignore-next-line: $currentUser is a stub for AccountProxyInterface.
      $currentUser,
    );
  }
