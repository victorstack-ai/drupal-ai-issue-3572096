...
class AiLoggingEventSubscriberTest extends UnitTestCase {
   ...
   */
  public function testWithLoggingDisabled(): void {
    ...
    // @phpstan-ignore-next-line: logger.factory is a stub with stubGetLogs().
   ...
   */
  public function testWithLoggingEnabled(): void {
    /** @var \Drupal\test_helpers\Stub\ConfigFactoryStub $configFactory */
    $configFactory = TestHelpers::service('config.factory');
    $configFactory->stubSetConfig(SettingsForm::CONFIG_NAME, [
   ...
   */
  public function testLoggingWithInputOutput(): void {
    /** @var \Drupal\test_helpers\Stub\ConfigFactoryStub $configFactory */
    $configFactory = TestHelpers::service('config.factory');
    $configFactory->stubSetConfig(SettingsForm::CONFIG_NAME, [
      SettingsForm::CONFIG_KEY_LOGGING_ENABLED => TRUE,
   ...
   */
  private function initAiLoggingEventSubscriberService(): AiLoggingEventSubscriber {
    /** @var \Drupal\test_helpers\Stub\ConfigFactoryStub $configFactory */
    $configFactory = TestHelpers::service('config.factory');
    // @phpstan-ignore-next-line: logger.factory is a stub with get().
    $loggerFactory = fn () => TestHelpers::service('logger.factory')->get('ai_observability');
    return new AiLoggingEventSubscriber(
      $configFactory,
      $loggerFactory,
    );
  }
